...
app.get("/make-server-c28f50fa/auth/check-admin", async (c) => {
  try {
    ...
    const hasAdmin = (data?.users?.length || 0) > 0;
...
app.post("/make-server-c28f50fa/auth/reset-password", async (c) => {
  try {
    ...
    // Verify reset code exists (user must have verified code first)
    const resetData = await kv.get(`reset:${email}`);
    if (!resetData) {
      return c.json({ error: 'Invalid reset session' }, 400);
    }
...
// Delete donation
app.delete("/make-server-c28f50fa/donations/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    const user = await getUser(accessToken);
    ...
    await kv.del(donationId);
    
    return c.json({ success: true });
  } catch (error) {
    console.log('Error deleting donation:', error);
    return c.json({ error: 'Failed to delete donation' }, 500);
  }
});
